...
function HabitTracker() {
  ...
  const toggleHabitCompletion = (id) => {
    setHabits(habits.map(habit => {
      if (habit.id === id) {
        ...
        if (todayLog) {
          // Toggle existing log
          const updatedLogs = habit.completionLog.map(log => 
            log.date.startsWith(today) ? { ...log, isComplete: !log.isComplete } : log
          );
          return { ...habit, completionLog: updatedLogs };
        } else {
          ...
          return { 
            ...
            completionLog: [
              ...habit.completionLog, 
              { date: today, isComplete: true, count: 1 }
            ] 
          };
        }
      }
  ...
  const incrementCount = (id) => {
    setHabits(habits.map(habit => {
      if (habit.id === id) {
        ...
        if (todayLog) {
          // Increment count in existing log
          const updatedLogs = habit.completionLog.map(log => 
            log.date.startsWith(today) ? { ...log, count: (log.count || 0) + 1, isComplete: true } : log
          );
          return { ...habit, completionLog: updatedLogs };
        } else {
          // Add new log for today with count 1
          return { 
            ...habit, 
            completionLog: [
              ...habit.completionLog, 
  ...
  // Function to calculate streak for each habit
  useEffect(() => {
    if (habits.length > 0) {
      const calculatedHabits = habits.map(habit => {
        ...
        const sortedLog = [...habit.completionLog].sort((a, b) => 
          new Date(b.date) - new Date(a.date)
        );
        
        // Calculate streak from completion log
        for (const log of sortedLog) {
          if (log.isComplete) {
            const currentDate = new Date(log.date);
            
            if (!previousDate) {
              // First completed log in the sequence
              currentStreak = 1;
              previousDate = currentDate;
            } else {
              // Check if dates are consecutive
              const timeDiff = previousDate - currentDate;
            ...
            // Streak broken by incomplete log
            break;
          }
        }
